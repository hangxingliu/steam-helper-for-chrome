...
function ca(a, c, e) {
	function f(a, b) {
		...
		let e = b + '',
		f = null;
		if (l && (f = JSON.parse(I.fromSource(e).toJSON()), e = I.removeMapFileComments(e)), N.babel.enable) {
			let a = ea(da(), k, e, f);
			if (a.err) return m(a.err);
			e = a.code, f = a.map;
		}
		try {
			q(c, e),
			l && d.writeFileSync(`${c}.map`, JSON.stringify(f, null, '\t'));
		} catch (a) {
	...
	function h(a) {
		let b = `    unknown error`.red;
		a && (a.message || a.stack) && (
		b = (a.message || a.stack).split(/[\n\r]+/).
		map((a) => `    ${a}`.red).join('\n')),
		console.error(`  error: browserify ${k}`.red + '\n' + b);
	}let k = j(c),l = N.source_map.enable && N.source_map.js,m = e,n = g([a], { extensions: ['.jsx'], debug: l, basedir: i(c), cache: {}, packageCache: {} });N.browserify.transform.map(({ name: a, options: b }) => n.transform(a, b)), L && (n.plugin(H, N.watchify), n.on('update', () => {m = () => (console.log(`${a} updated!`), Q.js()), n.bundle(f);})), n.bundle(f);
}
function da() {
